...
public class HttpRequestParser extends AbstractRequestParser {
	...
	protected void internalParseRequest()
		...
		this.virtualPathParams = requestUri.replaceAll("^/+", "")
				.replaceAll("/+$", "").split("/");

		if (virtualPathParams.length > 0) {
			requestUri = requestUri
					.substring(virtualPathParams[0].length() + 1);
		}
		...
		if (this.isMultipart) {
			...
			if ("POST".equalsIgnoreCase(httpRequest.getMethod())) {
				...
				try {
					...
					List<NameValuePair> formFields = URLEncodedUtils.parse(
							content, Charset.forName(encoding));
					for (NameValuePair formField : formFields) {
						this.formFields.put(formField.getName(),
								formField.getValue());
					}
				} catch (UnsupportedEncodingException e) {
