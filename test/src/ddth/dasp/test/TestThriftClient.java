...
public class TestThriftClient {
	static class ClientFactory implements
			PoolableObjectFactory<DaspJsonApi.Client> {
		@Override
		...
		BenchmarkResult result = new Benchmark(new Operation() {
			...
			public void run(int runId) {
				...
				try {
					...
					try {
						Object result = client.callApi(moduleName,
								functionName, jsonEncodedInput, authKey);
